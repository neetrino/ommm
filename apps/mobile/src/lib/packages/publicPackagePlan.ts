/** Public package plan returned by `GET /v1/packages/plans` (active plans only). */
...
export function normalizePublicPackagePlan(plan: PublicPackagePlan): PublicPackagePlan {
  ...
  return {
    ...
    freezeMaxDaysPerUse:
      typeof plan.freezeMaxDaysPerUse === "number"
        ? plan.freezeMaxDaysPerUse
        : 0,
    features: Array.isArray(plan.features) ? plan.features : [],
    typeSessionAllocations: Array.isArray(plan.typeSessionAllocations)
      ? plan.typeSessionAllocations.filter(
          (allocation): allocation is NonNullable<
            PublicPackagePlan["typeSessionAllocations"]
          >[number] =>

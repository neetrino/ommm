...
/** Studio wall-clock timezone (matches payment emails and public schedule). */
...
const STUDIO_WEEKDAY_TO_ENUM: Record<string, ScheduleDayOfWeek> = {
  Sun: 'SUNDAY',
  Mon: 'MONDAY',
  Tue: 'TUESDAY',
  Wed: 'WEDNESDAY',
...
const STUDIO_WEEKDAY_OFFSET: Record<ScheduleDayOfWeek, number> = {
  SUNDAY: 0,
  MONDAY: 1,
  TUESDAY: 2,
  WEDNESDAY: 3,
...
/** Day-of-week enum for an instant in the studio timezone. */
export function utcToStudioDayOfWeek(value: Date): ScheduleDayOfWeek {
...
/** End of the studio calendar day for `reference` (defaults to now). */
export function endOfStudioDay(reference: Date = new Date()): Date {
  const calendarDate = utcToStudioCalendarDate(reference);
  return studioWallClockToUtc(calendarDate, '23:59');
  // Note: DB queries use lte on startsAt; add 59s via endsAt helper below when needed.
}

...
import { AdminPackagePlanActions } from "@/components/admin/admin-package-plan-actions";
import { AdminPackagePlansShell } from "@/components/admin/admin-package-plans-shell";
import { AccountPageFrame } from "@/components/layout/account-page-frame";
import { formatAmdFromCents } from "@/lib/price-amd";
import { serverApiJson } from "@/lib/server-api";

type PackagePlanAdminRow = {
  id: string;
  name: string;
...
export default async function AdminMembershipPlansPage({
  ...
  const t = await getTranslations({ locale, namespace: "adminPages.membershipPlans" });
  const cookie = (await headers()).get("cookie") ?? "";
  const res = await serverApiJson<PackagePlanAdminRow[]>("/memberships/admin/plans", cookie);
            ...
            return (
              ...
              >
                ...
                <div className="mt-8 border-t border-white/50 pt-5">
                  ...
                  <AdminPackagePlanActions planId={plan.id} isActive={plan.isActive} />
                </div>
              </article>
            );
          })}

...
export async function MarketingPackagesPageContent({
  ...
  const res = await fetchPublicJsonCached<Plan[]>("/memberships/plans");
          ...
          return (
            ...
            >
              ...
              <p className="mt-6 font-serif text-3xl font-semibold tracking-tight text-sage-700">
                <span className="mr-1.5 text-black">
                  {amount.startsWith("֏") ? "֏" : ""}
                </span>
                {m("membershipsPriceLine", {
                  amount: amount.startsWith("֏")
                    ? amount.slice(1).trimStart()
                    : amount,
                })}
              </p>
      ...
      <section className="mt-16 max-w-3xl">
        ...
        <dl className="mt-6 space-y-6 text-sm text-sage-700">
          <div>
            ...
            <dd className="mt-1 text-sage-600">{m("membershipsFaqPauseAnswer")}</dd>
          </div>
          <div>
            <dt className="font-semibold text-sage-800">

...
type AdminGiftCardsEmptyStateProps = {
  filtersActive: boolean;
};

export function AdminGiftCardsEmptyState({ filtersActive }: AdminGiftCardsEmptyStateProps) {
  const t = useTranslations("adminPages.giftCards");
  const reducedMotion = usePrefersReducedMotion();
  const title = filtersActive ? t("filteredEmptyTitle") : t("emptyTitle");
  const description = filtersActive ? t("filteredEmptyDescription") : t("emptyDescription");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className="ommm-h2 mt-6 text-sage-800">{title}</h2>
        <p className="ommm-body-muted mx-auto mt-3 max-w-sm">{description}</p>
      </div>
    </motion.div>
  );
}

...
import { AdminWhatsappGatewayStatus } from '@/components/admin/admin-whatsapp-gateway-status';
import { AdminWhatsappLockedField } from '@/components/admin/admin-whatsapp-locked-field';
import { AdminWhatsappTestMessagePanel } from '@/components/admin/admin-whatsapp-test-message-panel';
...
export function AdminWhatsappSettingsForm({ initial }: AdminWhatsappSettingsFormProps) {
  ...
  const gatewayReady = settings.gatewayUrl.length > 0 && settings.hasToken;

  async function persist(): Promise<void> {
    setBusy(true);
    setMsg(null);
    try {
      const next = await apiFetch<WhatsappAdminSettings>('/whatsapp/admin/settings', {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    <AdminSectionShell banner={tone === 'ok' ? msg : null}>
      <div className="relative isolate grid gap-6 lg:grid-cols-2 lg:items-start">
        ...
        >
          ...
          <AdminWhatsappGatewayStatus enabled={gatewayReady} refreshKey={statusRefreshKey} />
        ...
        <AdminWhatsappConnectPanel canConnect={gatewayReady} />
      </div>
      <AdminWhatsappTestMessagePanel enabled={gatewayReady} />
      <OmmConfirmDialog
        isOpen={confirmOpen}

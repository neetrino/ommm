...
type AdminFinancePaymentsTotalProps = {
  locale: string;
...
export function AdminFinancePaymentsTotal({
  locale,
  totalAmountCents,
  totalCount,
}: AdminFinancePaymentsTotalProps) {
  const t = useTranslations("adminPages.finance.paymentsTab");

  return (
    <section className="flex justify-center px-1" aria-label={t("periodTotal")}>
      <article className={TOTAL_CARD_CLASS}>
        ...
        <p className="mt-2 text-xs font-medium text-sage-600">{t("rowCount", { count: totalCount })}</p>
      </article>
    </section>
  );
}

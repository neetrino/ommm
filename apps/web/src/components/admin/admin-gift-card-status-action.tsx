...
type AdminGiftCardStatusActionProps = {
  batchId: string;
  isActive: boolean;
  canToggle: boolean;
  labels: {
...
export function AdminGiftCardStatusAction({
  batchId,
  isActive,
  canToggle,
  labels,
  ...
  onBusyChange,
  onStatusMessage,
}: AdminGiftCardStatusActionProps) {
  const t = useTranslations("adminPages.giftCards");
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const [pendingConfirm, setPendingConfirm] = useState<PendingConfirm | null>(null);
  const isDisabled = disabled || busy || !canToggle;
  ...
  async function confirmStatusChange(): Promise<void> {
    ...
    try {
      await apiFetch(
        nextIsActive
          ? `/gift-cards/admin/batches/${batchId}/activate`
          : `/gift-cards/admin/batches/${batchId}/deactivate`,
        { method: "PATCH" },
      );

...
export const ADMIN_CLIENT_TAG_OVERLAY_BADGE_CLASS =
  `${TAG_OVERLAY_BASE_CLASS} -left-4 top-1 ${DEFAULT_TAG_OVERLAY_TILT_CLASS} rounded-sm px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[0.05em] shadow-[0_2px_8px_rgba(15,23,42,0.2)] ring-2 ring-white`;

/** Champagne ribbon — softer tilt, gold edge, no heavy stamp shadow. */
export const ADMIN_CLIENT_INFLUENCER_OVERLAY_BADGE_CLASS =
  `${TAG_OVERLAY_BASE_CLASS} -left-5 top-0 ${INFLUENCER_TAG_OVERLAY_TILT_CLASS} rounded-full border border-gift-gold/60 bg-gradient-to-r from-peach-100 via-paper to-peach-100 px-2 py-0.5 font-serif text-[8px] font-medium uppercase tracking-[0.14em] text-sand-700 shadow-[0_1px_4px_rgba(107,92,76,0.12)]`;

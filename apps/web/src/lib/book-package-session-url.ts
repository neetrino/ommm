...
export const BUY_PACKAGE_SESSION_QUERY_KEY = "buyPackageSession";
...
export function readBuyPackageSessionId(
  searchParams: URLSearchParams | Readonly<URLSearchParams>,
): string | null {
  const value = searchParams.get(BUY_PACKAGE_SESSION_QUERY_KEY)?.trim();
  return value && value.length > 0 ? value : null;
}

export function setBuyPackageSessionQuery(
  params: URLSearchParams,
  sessionId: string,
): void {
  params.set(BUY_PACKAGE_SESSION_QUERY_KEY, sessionId);
}

export function clearBuyPackageSessionQuery(params: URLSearchParams): void {
  params.delete(BUY_PACKAGE_SESSION_QUERY_KEY);
}

...
6. **Dev** — երկու առանձին terminal․ նախ `pnpm dev:api` (Nest, :4000), ապա `pnpm dev:web` (Next, :3000)։ Web-ը API-ին միանում է `.env`-ի `API_INTERNAL_URL` / `NEXT_PUBLIC_API_URL`-ով — մեկ հրամանով bundle պետք չէ։ `pnpm dev` — հուշում է հրամանները։ Մանրամասներ՝ [`MOBILE_SETUP.md`](./MOBILE_SETUP.md)։
